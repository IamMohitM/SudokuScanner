...
void detectEdges(cv::Mat& inputImage, cv::Mat& processedImage, const cv::Size& filterSize=cv::Size(5,5),
                 int cannyMinThresh=50, int cannyMaxThresh=200, int cannyAperture=3, bool blur=true, bool debug=false);
void drawHoughLines(cv::Mat& inputImage, int houghThresh=50, int houghMinLength=40, bool debug=false);
